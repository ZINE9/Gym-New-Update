...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $number_of_months = intval($select_option3);
    ...
    $updateSql = "UPDATE $table SET name = ?, phone = ?, weight = ?, weight_unit = ?, gender = ?, select_option2 = ?, select_option3 = ?, start = ?, end = ? WHERE id = ?";
    $stmt = $conn->prepare($updateSql);
    $stmt->bind_param('sssssssssi', $name, $phone, $weight, $weight_unit, $gender, $select_option2, $select_option3, $start_date, $end_date, $userId);

    if ($stmt->execute()) {
        header("Location: view_normal.php?table=$table&message=User updated successfully");
        exit();
    } else {
...
<body>
    <form method="POST" class="form-container">
        <h2>Edit <?php echo $table === 'client' ? 'Normal User' : 'Completed Payment User'; ?></h2><br>
        ...
        <a href="view_normal.php?table=<?php echo $table; ?>" class="back-link">Cancel</a>
    </form>
</body>
</html>
