...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($payment_status === 'done') {
        $stmt = $pdo->prepare('INSERT INTO done_normal (name, phone, weight, weight_unit, gender, select_option2, select_option3, start, end) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
    } else {
        $stmt = $pdo->prepare('INSERT INTO client (name, phone, weight, weight_unit, gender, select_option2, select_option3, start, end) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)');
    }

    $stmt->execute([$name, $phone, $weight, $weight_unit, $gender, $select_option2, $select_option3, $date, $endDate]);
